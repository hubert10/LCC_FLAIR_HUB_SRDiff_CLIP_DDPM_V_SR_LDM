...
class LatentDiffusion(DDPM):
    ...
    def apply_model(
        ...
        cond_lr: Optional[torch.Tensor] = None,
        ...
        x_recon = self.denoise_net(
            ...
            metadata=mtd,
            down_block_additional_residuals=cond_lr,  # conditioning with LR images
    ...
    def p_losses(
        ...
        for ts in range(img_lr_up.shape[1]):
            ...
            # print("--------------training-------------sr_dec----------------------:", sr_dec.shape)
        ...
        if self.parameterization == "x0":  # predicted image
            ...
            raise NotImplementedError()
        # print("********************************************************************")
        # print("sr_outputs:", sr_outputs.shape)
        # print("img_hr_down:", img_hr_down.shape)
        # # print("x:", x.shape)
        ...
        aux_loss = (
            ...
            * pixel_wise_closest_sr_sits_aer_loss(sr_outputs, img_hr_down, closest_idx)
            + hparams["grad_px_loss_weight"]
            * grad_pixel_wise_closest_sr_sits_aer_loss(
                sr_outputs, img_hr_down, closest_idx
            )
    ...
    def forward(
        self,
        img_hr: torch.Tensor,  # HR img_hr
        img_hr_down: torch.Tensor,  # downsampled img_hr
        img_lr: torch.Tensor,
        ...
        **kwargs,
        ...
        t = torch.randint(
            0, self.num_timesteps, (img_hr_down.shape[0],), device=self.device
        ...
        # from DiffusionSat x: 512x512 - 20 to cm z: 64x64 - 1.6 m
    ...
    def sample(
        self,
        img_hr,  # HR image used for conditioning
        img_hr_down,
        ...
        shape = img_hr_down.shape  # torch.Size([1, 4, 64, 64])
        ...
        for i in range(img_lr.shape[1]):
            ...
            latent = torch.randn(latent_shape, device=img_hr_down.device)

            for j, step in enumerate(iterator):
                latent, _ = ddim.p_sample_ddim(
                    x=latent,  # Random noise generated from encoded HR aerial image
                    cond_lr=[
                        feat[:, i] for feat in cond_lr
                    ],  # Encoded LR Sentinel-2 images
                    txt=txt,
                    mtd=mtd,
                    cond_hr=cond_hr,  # HR image
                    t=step,
                    index=custom_steps - j - 1,
